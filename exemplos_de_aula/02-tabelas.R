...
"/home/william/Documents/Curso-R/202205-intro-programacao\dados"
"C:\Users\willi\Documents\curso-r\202205-intro-programacao\dados"
...
base_de_dados_fev_base <- read.csv2("dados/voos_de_fevereiro.csv")
base_de_dados_fev_base

class(base_de_dados_fev)
class(base_de_dados_fev_base)
...
head(base_de_dados, n = 20)

# Últimas 6 linhas de uma tabela
tail(base_de_dados, n = 20) # tibbles vão ser mostradas até n = 20

tail(as.data.frame(base_de_dados), n = 23)

# transforma caracter em numero
...
paste("Em fevereiro, tivemos", nrow(base_de_dados_fev), "voos")
...
base_de_dados$saida_programada
...
base_de_dados$atraso_chegada[1:10]
...
# uma coluna
base_de_dados[1:10, c("origem", "companhia_aerea")]
...
select(base_de_dados, origem, companhia_aerea, atraso_chegada)
...
base_de_dados_fev$distancia

sum(base_de_dados$distancia)
sum(base_de_dados_fev$distancia)
...
as.POSIXct(1652745974, origin = "1970-01-01")


dia_hoje <- Sys.Date()
as.numeric(dia_hoje)

as.Date(19128, origin = "1970-01-01")

# lubridate, pacote com funções para manipular datas
